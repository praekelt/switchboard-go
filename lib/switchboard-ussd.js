...
function DummySwitchboardApi(im) {
    ...
    self.dob_to_obj = function(dob) {
        return {
            day: Number(dob.slice(0, 2)),
            month: Number(dob.slice(2, 4)),
            year: Number(dob.slice(4, 8))
        };
    };

    self.check_date_of_birth = function(user, dob) {
        return success(true);
    };
...
function SwitchboardApi(im, url, username, password) {
    ...
    self.obj_to_dob = function(obj) {
        var day = ("00" + obj.day.toString()).slice(-2);
        var month = ("00" + obj.month.toString()).slice(-2);
        var year = obj.year.toString();
        return day + month + year;
    };

    self._check_dob = function(dob, hw_birthdate) {
        if (!hw_birthdate) {
            return true;
        }
        return (dob == self.obj_to_dob(hw_birthdate));
    };

    self.check_date_of_birth = function(user, dob) {
        // TODO: currently disabled at SwB's request
        return success(true);
        var p;
        if (user.answers.registration_number &&
            user.answers.registration_number != '0') {
            // use mct-registrations for registration number
            p = self.api_get('mct-registrations', {
                registration: user.answers.registration_number
            });
            p.add_callback(function (result) {
                if (result.health_workers.length == 1) {
                    var hw = result.health_workers[0];
                    return self._check_dob(dob, hw.birthdate);
                }
                return false;
            });
        }
        else if (user.answers.cheque_number &&
                 user.answers.cheque_number != '0') {
            // use mct-payrolls for check number
            p = self.api_get('mct-payrolls', {
                check: user.answers.cheque_number
            });
            p.add_callback(function (result) {
                if (result.mct_payrolls.length == 1) {
                    var hw = result.mct_payrolls[0];
                    return self._check_dob(dob, hw.birthdate);
                }
                return false;
            });
    ...
    self.register_health_worker = function(health_worker) {
        var p = self.api_post('health-workers', {
            ...
            birthdate: health_worker.birthdate, // {day: 1, month: 2, ...}
            country: health_worker.country, // string
            email: health_worker.email, // string
            ...
            // other_phone: health_worker.other_phone, // string, MSISDN
            // address: health_worker.address, // string, postal address
            language: self.lang
        });
        return p;
    };
...
function RegisterHealthWorker() {
    ...
    self.incr_metric = function(im, metric) {
        ...
        p.add_callback(function (value) {
            im.metrics.fire_max('first_session_completed', value);
        });
        im.api.request(
    ...
    self.send_sms_session2_end = function(im) {
        var _ = im.i18n;
        var msg = _.gettext("Thank you for registering with The Health Network" +
                            " Programme. We will process your registration and" +
                            " confirm with you when you are registered.");
        return self.send_sms(im, msg);
    };
    ...
    self.on_session_new = function(event) {
        event.im.metrics.fire_inc('ussd_sessions');
        event.im.metrics.fire_inc('session_new_in.' +
                                  event.im.current_state.name);
        self.inc_user_item(event.im.user, 'ussd_sessions');
    };

    self.on_session_close = function(event) {
        event.im.metrics.fire_inc('session_closed_in.' +
                                  event.im.current_state.name);
        if (event.data.possible_timeout) {
            event.im.metrics.fire_inc('possible_timeout_in.' +
                                      event.im.current_state.name);
    ...
    self.on_new_user = function(event) {
        event.im.metrics.fire_inc('unique_users');
    };

    self.on_state_enter = function(event) {
        event.im.metrics.fire_inc('state_entered.' + event.data.state.name);
    };

    self.on_state_exit = function(event) {
        event.im.metrics.fire_inc('state_exited.' + event.data.state.name);
    };
    ...
    // Session 1
        ...
        self.add_state(new LanguageChoice(
            ...
            _.gettext("Welcome to the Health Network brought to you by" +
        ...
        self.add_state(new FreeText(
            "cadre_other",
            "cadre_unavailable",
            _.gettext("Please specify your CADRE:")
        ));
        self.add_state(new ChoiceState(
            ...
            _.gettext("Sorry but this service is not yet available for" +
                      " your CADRE. Would you like us to contact you if this" +
                      " service becomes available for you in the future?"),
            [
        ...
        self.add_state(new FreeText(
            ...
            _.gettext("If u R a government Health Worker please enter the" +
                      " last 7-9 digits of Ur cheque no (eg 1234567)." +
            ...
            _.gettext("Sorry but that is not a valid cheque number." +
                      " Please try again of enter '0' if you do not" +
        ...
        self.add_state(new FreeText(
            ...
            // "date_of_birth",
            "first_name",
            _.gettext("To verify that u r a registered health worker enter" +
        ...
        // Note: Date of birth state disabled at SwB's request
        self.add_state(new FreeText(
            "date_of_birth",
            function (content, done) {
                var swb_api = self.swb_api(this.im);
                var p = swb_api.check_date_of_birth(this.im.user, content);
                p.add_callback(function (result) {
                    if (result) {
                        done("first_name");
                    }
            ...
            _.gettext("Please enter your date of birth in the following" +
                      " format DDMMYYYY (eg if born on 23 April 1980," +
                      " enter 23041980)."),
            function (content) {
                if (!content.match("^[0-9]{8}$"))
                    return false;
                var day = Number(content.slice(0, 2));
                var month = Number(content.slice(2, 4));
                var year = Number(content.slice(4, 8));
                var year_now = (new Date()).getFullYear();
                if (day > 31 || month > 12 || year_now - year < 16)
                    return false;
                return true;
            },
            _.gettext("Sorry but you have not entered a valid date." +
                      " Please enter your date of birth in the following" +
                      " format DDMMYYYY (e.g. if born on 23 April 1980," +
                      " enter 23041980).")
        ));
        self.add_state(new ChoiceState(
            "dont_match_mct",
            ...
            _.gettext("Sorry but the registration number and DOB that you" +
                      " have entered do not match with MCT records."),
            [
        ...
        self.add_state(new EndState(
            ...
            "session2_intro",
            {
                on_enter: function () {
                    var im = this.im;
                    self.incr_metric(im, 'first_session_completed');
                    self.send_sms_session1_end(im);
                }
            }
        ));
    ...
    // Session 2
        ...
        self.add_state(new FreeText(
            ...
            _.gettext("Welcome back 2 the Health Network brought 2 U by " +
                      "Switchboard, the Ministry of Health and Vodacom." +
                      "\n\n" +
                      "Please enter your district (eg Kilosa). No" +
        ...
        // Note: Email state disabled at SwB's request
        self.add_state(new FreeText(
            "email",
            "session2_end",
            _.gettext("Please enter your email address or 0 if you do not " +
                      "have an email address"),
            function (content) {
                return Boolean(content.match("^(.+\\@.+\\..+|[0Oo])$"));
            },
            _.gettext("Sorry but that is not a valid email address. Please" +
                      " try again or enter 0 if you do not have an email" +
                      " address.")
        ));
        // Note: confirmation SMSes need to be sent manually via Vumi Go
        //       once the registration has been approved and activated.
        ...
        self.create_health_worker = function (im) {
            ...
            var date_of_birth = ans("date_of_birth");
            if (date_of_birth)
                hw.birthdate = swb_api.dob_to_obj(date_of_birth);
            hw.country = "TZ";
            var email = ans("email");
            hw.email = (email && !email.match('^[0Oo]$')) ? email : null;
            hw.facility = ans("facility_select");
        ...
        self.add_state(new EndState(
            ...
            },
            {
                on_enter: function () {
                    ...
                    swb_api.register_health_worker(health_worker);
                    self.set_user_item(im.user, "registered", 1);
                    self.send_sms_session2_end(im);
                    self.incr_metric(im, 'second_session_completed');
                    var sessions = self.get_user_item(im.user,
                                                      'ussd_sessions', 0);
                    im.metrics.fire_avg('sessions_taken_to_register',
                                        sessions);
                }
            }
        ));
